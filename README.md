...
8. **Ajuste de curvas.** El contorno se parte en las esquinas y cada tramo se ajusta con tangentes de un solo lado, de modo que los vértices quedan vivos. Los tramos que resultan rectos se emiten como líneas y los colineales consecutivos se funden.
9. **Eliminación de nodos.** El ajuste recursivo parte por la mitad en cuanto el error se pasa y nunca vuelve atrás, así que cada partición deja un nodo permanente aunque después resulte innecesario. Una pasada posterior intenta borrar cada unión y reajustar el tramo unido, quedándose con el borrado sólo si el error sigue dentro de tolerancia.
